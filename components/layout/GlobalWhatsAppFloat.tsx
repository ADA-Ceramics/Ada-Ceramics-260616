...
export function GlobalWhatsAppFloat() {
  ...
  const isHome = localeStripped.length === 0
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex items-center gap-3">
      {/* 首页专属提示气泡 */}
      {isHome && showTip && (
        <div className="hidden sm:flex items-center gap-2 rounded-xl bg-white shadow-lg ring-1 ring-black/5 px-4 py-2">
          <span className="text-sm font-medium text-[#1a1a2e]">Instant OEM Quote</span>
          <button
            type="button"
      ...
      <a
        ...
        className="group flex items-center gap-3 rounded-xl bg-[#f5efe6] p-2 shadow-lg ring-1 ring-black/5 transition-all duration-200 hover:-translate-y-[3px] hover:shadow-[0_10px_24px_-6px_rgba(37,211,102,0.45)]"
      >
        {/* 官方绿底白对话气泡图标 48x48 */}
        <span className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-[#25D366] text-white">
        ...
        {/* 白色圆角按钮文字：移动端隐藏 */}
        <span className="hidden sm:inline-flex items-center rounded-lg border border-[#25D366] bg-white px-4 py-2 text-sm text-[#3f3f46]">
